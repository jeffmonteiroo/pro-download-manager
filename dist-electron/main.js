...
class CookieManager {
  constructor() {
    __publicField(this, "BLOCK_MEMORY_DURATION", 60 * 60 * 1e3);
  }
  // 1 hour
  /**
   ...
   */
  setBrowser(browser) {
    store$2.set("cookie_browser", browser);
  }
  /**
  ...
  getStatus() {
    ...
    if (elapsed < this.BLOCK_MEMORY_DURATION) {
      return {
        blocked: true,
        minutesAgo: Math.floor(elapsed / 6e4)
      };
    }
    return { blocked: false, minutesAgo: null };
  }
  ...
  isBotError(error2) {
    var _a;
    const msg = ((_a = error2.message) == null ? void 0 : _a.toLowerCase()) || "";
    return msg.includes("sign in") || msg.includes("bot") || msg.includes("cookies") || msg.includes("authentication");
  }
}
...
class LogManager {
  // 7 days
  constructor() {
    __publicField(this, "MAX_AGE_NORMAL_MS", 24 * 60 * 60 * 1e3);
    // 24 hours
    __publicField(this, "MAX_AGE_ERROR_MS", 7 * 24 * 60 * 60 * 1e3);
  ...
  getLogDirectory() {
    if (app$1.isPackaged) {
      return path.join(app$1.getPath("userData"), "logs");
    } else {
    ...
    try {
      ...
      files.forEach((file) => {
        ...
        try {
          ...
          const content2 = fs.readFileSync(filePath, "utf8").substring(0, 1e3);
          const hasError = content2.includes("ERROR") || content2.includes("failed");
...
class DownloadEngine extends EventEmitter {
  constructor(win2) {
    ...
    __publicField(this, "maxConcurrent", 3);
    this.win = win2;
    this.loadFromStore();
    this.setupIPC();
  }
  setupIPC() {
    try {
      ipcMain$1.removeHandler("select-directory");
    } catch (e) {
    }
    ...
    ipcMain$1.removeAllListeners("pause-download");
    ipcMain$1.removeAllListeners("resume-download");
    ipcMain$1.removeAllListeners("cancel-download");
    ipcMain$1.on("pause-download", (_event, id2) => this.pauseDownload(id2));
    ipcMain$1.on("resume-download", (_event, id2) => this.resumeDownload(id2));
    ipcMain$1.on("cancel-download", (_event, id2) => this.cancelDownload(id2));
  }
  loadFromStore() {
    const saved = store$1.get("downloads") || [];
  ...
  saveToStore() {
    store$1.set("downloads", this.queue);
  }
  ...
  processQueue() {
    ...
    const nextTask = this.queue.find((t2) => t2.status === "pending");
    if (nextTask) {
  ...
  downloadWithYtDlp(task) {
    ...
    if (task.format === "audio") {
      ...
      if (task.audioQuality) {
        args.push("--audio-quality", task.audioQuality);
      }
    } else {
      if (task.resolution) {
...
`);
      ...
      if (line.startsWith("[download]")) {
        ...
        const progress = percentMatch ? percentMatch[1] : void 0;
        ...
        if (progress) {
          ...
          let eta = etaMatch ? etaMatch[1] : void 0;
          if (line.includes("100%")) eta = "00:00";
          console.log(`[DEBUG ${task.id}] CALLING updateTaskProgress with:`, {
            progress,
            speed: speedMatch ? speedMatch[1] : void 0,
            eta,
            size: sizeMatch ? sizeMatch[1] : void 0
          });
          this.updateTaskProgress(
            task.id,
            progress,
            speedMatch ? speedMatch[1] : void 0,
            eta,
            sizeMatch ? sizeMatch[1] : void 0
          );
        }
      }
    });
  ...
  updateTaskProgress(id2, progress, speed, eta, totalSize, status) {
    const task = this.queue.find((t2) => t2.id === id2);
    if (task) {
      ...
      console.log(`[DEBUG ${id2}] Sending to renderer:`, payload);
      this.win.webContents.send("download-progress", payload);
      this.emit("download-progress", payload);
    } else {
      console.log(`[DEBUG ${id2}] Task NOT FOUND in queue!`);
    }
  }
  handleProcessExit(task, proc, finalPath, getErrorLog) {
    proc.on("close", (code2) => {
  ...
  pauseDownload(id2) {
    const proc = this.activeDownloads.get(id2);
    if (proc) {
      proc.kill();
    ...
    if (task) {
      task.status = "paused";
      this.win.webContents.send("download-progress", { id: id2, status: "paused", progress: task.progress });
      this.saveToStore();
    }
    this.processQueue();
  }
  resumeDownload(id2) {
    ...
    if (task) {
      task.status = "pending";
      this.win.webContents.send("download-progress", { id: id2, status: "pending", progress: task.progress });
      this.saveToStore();
      this.processQueue();
    }
  }
  cancelDownload(id2) {
      ...
      if (task && (task.status === "downloading" || task.status === "converting")) {
        try {
          ...
          const matchingFiles = allFiles.filter(
            (file) => file.startsWith(task.filename) && (file.endsWith(".mp4") || file.endsWith(".webm") || file.endsWith(".mp3") || file.endsWith(".part") || file.endsWith(".ytdl") || file.endsWith(".temp") || file.includes(".f") || file.includes(".part-"))
            // yt-dlp fragment files
          );
    ...
    this.queue = this.queue.filter((t2) => t2.id !== id2);
    this.saveToStore();
    this.processQueue();
    this.win.webContents.send("download-removed", { id: id2 });
  }
  getInitialState() {
    return this.queue;
  }
  cancelTasksByPrefix(prefix) {
    console.log(`[ENGINE] Bulk canceling tasks with prefix: ${prefix}`);
    const tasksToCancel = this.queue.filter((t2) => t2.id.startsWith(prefix));
    tasksToCancel.forEach((t2) => this.cancelDownload(t2.id));
  }
}
class VideoAnalyzerService {
  ...
  async runAnalysis(url, useCookies) {
    return new Promise((resolve2, reject) => {
      ...
      proc.on("close", (code2) => {
        ...
        try {
          ...
          const formats2 = (data.formats || []).map((f) => ({
...
class PlaylistAnalyzerService {
   ...
   */
  isPlaylistUrl(url) {
    return url.includes("list=") || url.includes("/playlist") || url.includes("/sets/") || url.includes("soundcloud.com/") && !url.includes("/s-");
  }
  /**
    ...
    return new Promise((resolve2, reject) => {
      ...
      proc.on("close", (code2) => {
        ...
        try {
          ...
          const videoList = videos.map((v, index) => ({
            id: v.id || v.url,
            playlistId,
            ...
            filesize: v.filesize,
            url: v.url
            // ✅ CRITICAL: Pass the original URL for download
          }));
          const metadata2 = {
            id: playlistId,
          ...
          console.log(`[PlaylistAnalyzer] Found ${videos.length} videos in playlist: ${metadata2.title}`);
          resolve2({
            ...metadata2,
            videos: videoList
          });
...
class BatchDownloadManager {
  constructor(engine2) {
    ...
    __publicField(this, "activeDownloadsCount", 0);
   ...
   */
  setConcurrentLimit(limit2) {
    if (limit2 < 1 || limit2 > 9) {
      throw new Error("Concurrent downloads must be between 1 and 9");
    }
    this.concurrentLimit = limit2;
    store.set("settings.concurrentDownloads", limit2);
    console.log(`[BatchManager] Concurrent limit set to ${limit2}`);
  }
  /**
  ...
  async startPlaylistDownload(playlist) {
    ...
    selectedVideos.map(
      (video) => this.queueVideo(video, playlist).catch((err) => {
   ...
   */
  async queueVideo(video, playlist) {
    return new Promise((resolve2, reject) => {
      this.downloadQueue.push({ video, playlist, resolve: resolve2, reject });
      this.processQueue();
    });
  }
  /**
  ...
  async processQueue() {
    ...
    try {
      ...
      console.error(`[BatchManager] Download failed:`, error2);
      item.reject(error2);
    } finally {
      this.activeDownloadsCount--;
      this.processQueue();
  ...
  */
  async downloadVideo(video, playlist) {
    ...
    const task = {
      ...
      // ✅ Unique ID
      url: taskUrl,
      // Use correct URL (SoundCloud url or YouTube constructed)
      type: "standard",
  ...
  generateVideoFilename(video) {
    ...
    const title2 = this.sanitizeFilename(video.title);
    return `${index} - ${title2}`;
  }
  /**
...
function setupIPCHandlers() {
  ...
  try {
    ipcMain$1.removeHandler("analyze-url");
    ipcMain$1.removeHandler("analyze-playlist");
  } catch (e) {
  }
  ipcMain$1.handle("analyze-url", async (_event, url) => {
    try {
  ...
  ipcMain$1.handle("analyze-playlist", async (_event, url) => {
    try {
...
function createWindow() {
  ...
  if (!engine) {
    ...
    ipcMain$1.on("start-download", (_event, task) => {
